...
// host binding selection, work mutation, or HTML rendering.
...
impl WorkListQuery {
    fn validate(&self) -> Result<(), ApiError> {
        ...
        if self.after.is_some() && self.reader_session_id.is_none() {
            return Err(ApiError::bad_request(
                "Work continuation requires its readerSessionId",
            ));
        }
        Ok(())
    }
    ...
    fn arguments(&self) -> Vec<String> {
        let mut args = vec!["ls", "--all", "--verbose", "--json", "--limit", "20"]
            .into_iter()
            .map(str::to_owned)
            .collect::<Vec<_>>();
        for (flag, value) in [
