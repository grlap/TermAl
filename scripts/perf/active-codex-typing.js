...
const OUT_PATH = ".tmp/active-codex-typing-profile.json";
const TEXT = "typingprofiletest";
const SETTLE_MS = 900;
const BETWEEN_KEYS_MS = 40;
  ...
  try {
    ...
    const tabInfoResult = await send("Runtime.evaluate", {
      expression: `(() => {
        ...
        return {
          codexTab: tabs.find((tab) => /codex/i.test(tab.label)) ?? null,
        };
      })()`,
      returnByValue: true,
    });
    ...
    if (!tabInfo?.codexTab) {
      throw new Error("Need a visible Codex session tab for the active typing profile.");
    }
    if (!tabInfo.codexTab.selected) {
      const targetX = tabInfo.codexTab.rect.left + tabInfo.codexTab.rect.width / 2;
      const targetY = tabInfo.codexTab.rect.top + tabInfo.codexTab.rect.height / 2;
    ...
    const setupResult = await send("Runtime.evaluate", {
      expression: `(() => {
        if (window.__termalActiveTypingProfile?.cleanup) {
          window.__termalActiveTypingProfile.cleanup();
        }
        const activePane = document.querySelector('.workspace-pane.active');
        const tablist = activePane?.querySelector('[role="tablist"][aria-label="Tile tabs"]');
        ...
        const state = {
          ...
          bodyHasCodexWorking: bodyText.includes('Codex is working'),
          bodyHasWaitingForChunk: bodyText.includes('Waiting for the next chunk of output'),
        };
        state.cleanup = () => {};
        target.focus();
        target.setSelectionRange(target.value.length, target.value.length);
        window.__termalActiveTypingProfile = state;
        return {
          activeTabLabel: state.activeTabLabel,
          bodyHasCodexWorking: state.bodyHasCodexWorking,
          bodyHasWaitingForChunk: state.bodyHasWaitingForChunk,
    ...
    if (!setup || setup.error) {
      throw new Error(setup?.error || "Failed to prepare active typing profiling.");
    }
    ...
    for (const char of TEXT) {
      ...
      await send("Runtime.evaluate", {
        expression: `(async () => {
          const state = window.__termalActiveTypingProfile;
    ...
    const captureResult = await send("Runtime.evaluate", {
      expression: `(() => {
        ...
        return {
          ...
          samples: state.samples.filter(Boolean),
        };
      })()`,
      returnByValue: true,
    });
    ...
    const restoreResult = await send("Runtime.evaluate", {
      expression: `(() => {
        ...
        state.cleanup?.();
        delete window.__termalActiveTypingProfile;
        return true;
      })()`,
      returnByValue: true,
    });

    const delays = (capture?.samples ?? [])
      .map((sample) => sample.nextFrameDelay)
      .filter((value) => typeof value === "number");
    const summary = {
      keystrokes: delays.length,
      averageNextFrameDelayMs:
        delays.length > 0
          ? round(delays.reduce((sum, value) => sum + value, 0) / delays.length, 2)
          : null,
      maxNextFrameDelayMs:
        delays.length > 0 ? round(Math.max(...delays), 2) : null,
      keystrokesOver16Ms: delays.filter((value) => value > 16).length,
      keystrokesOver32Ms: delays.filter((value) => value > 32).length,
    };
