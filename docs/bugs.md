...
**Severity:** Low - `ui/src/SessionPaneView.tsx` is still 3,679 lines and `ui/src/app-session-actions.ts` is still about 2,290 lines after the latest small helper splits, both past the architecture rubric thresholds (~2,000 for TSX components, ~1,500 for utility modules).

The waiting-indicator helpers now live in `ui/src/SessionPaneView.waiting-indicator.ts`, the session-settings optimism helpers now live in `ui/src/app-session-settings-optimism.ts`, and conversation-marker response matching now lives in `ui/src/conversation-marker-response-match.ts`. Those moves reduced local clutter and gave the helpers direct unit-testable surfaces, but the main production modules remain over threshold.
