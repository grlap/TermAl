...
Also fixed in the current tree: terminal delegation refresh clears completed/failed child queues before lifecycle dispatch can run a follow-up prompt; parent removal detaches and shuts down impacted child runtimes without publishing parent-card deltas for the removed session; read-only delegation guards now cover project/workdir-scoped writes without `sessionId` and reject approval acceptance; read-only write checks now evaluate session and project/workdir scopes cumulatively and clone delegated scopes before path canonicalization so disk I/O stays outside the state mutex; delegation creation trims empty model overrides back to the agent default; terminal child dispatch re-checks delegation status under the dispatch lock to close the create/cancel race; `DelegationWritePolicy` accepts legacy snake_case discriminators for persisted records; targeted equal-revision repair retries now use the existing reconnect backoff; failed-start delegation responses re-fetch the post-failure child snapshot; whitespace-only delegation prompts are rejected; duplicate terminal cancel no longer commits a no-op revision; delegation constants / broad workdir matching / terminal gate ordering are documented inline; cancel/fail queue cleanup now uses the shared helper; resync option coalescing uses the stricter revision floor with explicit null-to-undefined narrowing and false-to-true reconnect-confirmation coverage; and read-only delegation write checks derive project/workdir scope from `sessionId` and return a title-bearing 403 while the architecture docs describe the project-wide fail-closed behavior.

Also fixed in the current tree: delegation REST endpoints and lifecycle SSE events are now documented in `docs/architecture.md`; the delegation brief explains that `queued` is reserved for a future scheduler while Phase 1 REST spawn creates `running` records or returns `409`; related feature briefs now cross-link back to delegation sessions; the cancel route's idempotent terminal response contract is documented; the virtualized conversation handle ref no longer allocates an ignored state object every render; the stale scroll-cooldown test comment now names `VIRTUALIZED_USER_SCROLL_ADJUSTMENT_COOLDOWN_MS`; no-op terminal delegation cancel now has revision/delta stability coverage; read-only workdir containment is covered in both nested and ancestor directions; and delegation delta-pump tests now drain until `try_recv()` is empty instead of panicking after fixed loop counts.

Also fixed in the current tree: mixed explicit `projectId` / `workdir` and `sessionId` requests now evaluate both the explicit and session-derived scopes, so an unrelated explicit scope cannot hide a parent or sibling session's active read-only delegation; terminal routes now run only the cheap session-level delegation check before remote routing and defer path-scoped local checks to the blocking pool after remote/local routing is known; the read-only guard no longer reports a literal "unknown delegation" title, trims session-derived workdirs, ignores terminal read-only delegations on the session branch, and uses scope-neutral 403 wording; remaining delegation constants are annotated; the HTTP workdir-only terminal route path is covered; and the new delegation feature backlink uses the same ASCII separator style as sibling links.

Also fixed in the current tree: session settings now use the session-only read-only delegation guard so parent and sibling settings updates are not blocked by project-wide delegation locks while child settings remain blocked; mutable file/git/review write routes run the same cheap child-session guard before remote proxy routing; explicit local `projectId` targets now contribute their project root to read-only containment checks, closing overlapping-project bypasses while preserving unrelated-project writes; and delegation creation docs now distinguish handler-level `400` validation from extractor-level `422` JSON shape failures.

Also fixed in the current tree: delegation creation now rejects remote-proxy parent sessions even when they have no mapped local project id, so unsupported remote-backed delegations cannot spawn local children; malformed or schema-invalid delegation create JSON now maps `JsonRejection` into the standard `{ "error": ... }` API response with `422` status; and both paths have route/state regression coverage.

Also fixed in the current tree: read-only delegation write checks now expand explicit and workdir-derived local project roots after releasing the state lock, so sibling workdirs under the same local project are blocked; Git write routes also check the resolved repository root and file-action pathspec targets before spawning Git; missing child sessions for still-running read-only delegations now fall back to the delegation cwd instead of failing open; delegation create JSON rejection preserves non-schema extractor statuses such as missing content type while keeping syntax/data errors at `422`; the virtualized conversation handle ref now has an explicit initialized-or-null type and a stable reader callback instead of an `undefined as unknown` assertion; duplicate delegation JSON rejection coverage was consolidated; empty-string delegated workdir matching is covered alongside whitespace-only workdirs; terminal read-only child gates now document the Phase-1 local-only invariant; stale child delegation links document the intentional anomaly-signaling write block; and completed-delegation cancel tests now assert the pre-cancel `DelegationCompleted` delta before proving duplicate cancel is delta-stable.

Also fixed in the current tree: failed delegation start cleanup now skips child-runtime detach and persistence commits when the delegation is already terminal; and read-only delegation write checks now skip delegated-scope collection entirely when the session-only guard already blocks the write.

Also fixed in the current tree: same-revision delegation sibling deltas are replayed on the frontend for idempotent session-delta shapes, so parent-card/session-created details are not lost when the matching state snapshot revision is already current; completed, canceled, and failed read-only delegations now have explicit child-write unblock coverage; and terminal failed-start cleanup is covered as a no-op with no revision or delta emission.
