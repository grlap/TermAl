...
- The coordination board currently has no human-facing panel. Agents access
  it through the `termal_board_*` MCP tools or the project-scoped HTTP routes.
  This keeps its level-triggered coordination role distinct from the visual
  [Response Board](response-board.md), which stores immutable transcript-card
  snapshots for a human workspace.
