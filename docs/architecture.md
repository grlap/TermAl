...
4. On tool approval needed → Claude sends `control_request { subtype: "can_use_tool" }` → TermAl either auto-approves or shows approval card → sends `control_response` with decision — except `AskUserQuestion`, whose questions ride this same request: TermAl shows the question card in every attended mode and returns the answers in the allow decision's `updatedInput.answers`, keyed by the exact question text — a label string for single-select, a label array for multi-select. Skipping the card returns a deny that tells Claude to decide on its own. Unattended sessions — read-only reviewer delegations, and delegation children running under AutoApprove or Plan — get that decide-without-asking deny immediately instead of a question card nobody would answer, and the transcript records a resolved (`declined`, non-declinable) question card noting that TermAl asked Claude to decide without human input; if the question payload is malformed, the transcript records its parser diagnostic instead because no valid question card can be built. A fourth self-resolution attempt in one turn terminates the Claude runtime and fails the turn instead of leaving its control request parked or looping indefinitely. Root AutoApprove/Plan sessions and an Ask-mode implementer child stay attended (child identity alone is not the signal, writable children are never forced into Ask, and opening a pane does not override the chosen child policy). The compatibility-only `request_user_dialog` transport has no deny envelope, so its unattended fallback returns a protocol control error carrying the same instruction and records that limitation on the audit card; current Claude CLIs no longer emit that channel, so this fallback is not backed by the live permission-transport capture. For the same reason, legacy dialog multi-select answers retain their historical comma-joined string encoding instead of adopting the permission transport's live-verified label array.
   **Decline semantics** (`POST /api/sessions/{id}/user-input/{message_id}` with `declined: true`): `answers` must be empty or omitted — a decline carrying answers is a 400, validated before any pending lookup or remote proxying, so local and proxied sessions reject identically; only Claude cards that arrived over the permission channel are declinable (`declinable: true` on the card) and resolve as `declined` — Codex `request_user_input`, the legacy Claude dialog channel, and Cursor/Gemini/OpenCode (no structured input at all) answer 409
