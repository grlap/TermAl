...
| GET | `/api/test-runs` | Read-only list of test-launcher runs the host indexed from each local project's Git `review-runs` directories (main and linked worktrees), newest first. Optional `projectId` filter. Every non-terminal run plus at most 50 terminal runs per project. The same summaries ride `StateResponse.testRuns` and the `testRunChanged`/`testRunRemoved` deltas. See [test runs](features/test-runs.md). |
