...
| POST | `/api/terminal/run/stream` | Run the same terminal command as `/api/terminal/run`, but return an SSE stream with `output` events for stdout/stderr chunks and a final `complete` event carrying the normal terminal response. There is no process timeout. Remote-scoped commands proxy this streamed route when the remote supports it and fall back to the JSON route for older remotes. |
