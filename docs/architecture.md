...
| POST | `/api/sessions/{id}/acceptance-evaluations` | Ask the host to produce the acceptance evaluation an Engram task needs. Body: `workRef`, optional `agent` (Claude or Codex) and `model`; unknown fields, including any caller prompt, are rejected. Reads the task and the policy as the host reader, selects the mode, and either spawns a read-only `evaluator` delegation (201: the delegation creation response plus `mode`, `workRef`) or returns a `same_session` brief (200). 400 invalid reference or evaluator agent; 404 missing session; 409 no enabled Engram project, task not open or without criteria, or a mode the policy does not admit; 429 busy; 501 `sub_agent`; 502 tracker read failure. See [acceptance evaluation](features/engram-host-adapter.md#acceptance-evaluation). |
| POST | `/api/sessions/{id}/acceptance-evaluation` | Active local read-only evaluator child only. Body: `schemaVersion` 1 and one verdict per criterion (`criterion`, `verdict`, optional `basis`, `rationale`, optional `evidence` locators). The host derives the task, mode, bases, identity, model and attempt key, and records the evaluation in Engram as the child. 200 returns the stored receipt; 400 invalid verdicts; 422 unknown fields/type errors; 409 no evaluator authority, an evaluation already recorded, or Engram's own refusal text; 429 busy; 502 deadline, transport, locked store or unreadable receipt, with nothing recorded. See [acceptance evaluation](features/engram-host-adapter.md#acceptance-evaluation). |
